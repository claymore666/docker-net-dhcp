...
[`.github/workflows/pages.yml`](https://github.com/claymore666/docker-net-dhcp/blob/main/.github/workflows/pages.yml).
...
[`README.md`](https://github.com/claymore666/docker-net-dhcp/blob/main/README.md)
...
[`scripts/test-actionlint.sh`](https://github.com/claymore666/docker-net-dhcp/blob/main/scripts/test-actionlint.sh)
asserts the linter still catches this exact bug class). Second line: the
[**rc-tag dry-run**](#pre-release-dry-run-rc-tags) exercises the whole
...
[`scripts/assert-newest-release-tag.sh`](https://github.com/claymore666/docker-net-dhcp/blob/main/scripts/assert-newest-release-tag.sh)
...
- **`:latest` is still untouched, and that is now asserted from the
  registry**. The assertion no longer infers it from the skip: the run
  reads `:latest` before it starts and re-reads it at the end. If the
  redirect were ever mistyped, the rc would fail, and it would not ship
...
whole procedure. Nothing to mint, nothing to launch. If the lane reports
no runner anyway, the board is down. The fix belongs to the host, and no
release step will start it.
...
README,
[`docs/`](https://github.com/claymore666/docker-net-dhcp/tree/main/docs),
...
2. **Bump install pins:** `scripts/bump-version.sh vX.Y.Z` (#251). It
   ...
   and
   [`docs/`](https://github.com/claymore666/docker-net-dhcp/tree/main/docs))
   to the new tag, and leaves bare `vX.Y.Z` feature markers and
   historical prose (`As of vPREV every PR...`, `v1.1.0 onward`) alone.
   The image ref is what tells a pin from prose. Verify with `git diff`
   and
...
3. **Documentation review, PR-driven against the milestone.** Don't
   ...
   [`docs/parent-attached-modes.md`](parent-attached-modes.md));
   ...
   [`GOVERNANCE.md`](https://github.com/claymore666/docker-net-dhcp/blob/main/GOVERNANCE.md)
   and
   ...
   **Some drift belongs to no milestone PR at all**, so the per-PR read
   ...
   - **Mechanisms this release added that no page describes.** The
     ...
     `internals.md` covers each; v1.6.0 shipped a lease reclaim and a
     per-parent gate with no section for either, while every counter
     table was green.
   - **Standing preconditions that read like old-version prose.** The
     `BREAKING CHANGE IN v1.5.0` block
     ([`README.md`](https://github.com/claymore666/docker-net-dhcp/blob/main/README.md),
     [`docs/index.md`](index.md), [`docs/reference.md`](reference.md))
     is a precondition for *every* install and never a changelog entry,
     but it names an old version, so a pass tidying stale version
   ...
   - **Counted claims.** "Four flip `healthy` to `false`" is right until
     ...
     **This bullet's own example went stale while it was being used as
     ...
     (#638)", and v1.8.0 added a fifth counter: the gate held every
     counter *list* it read, and the doc still shipped rows saying
     "four" over a list of five, because the gate read the count word in
     one sentence and the file states it in five (#724). The claim was
     not enforced. A narrower claim was, and the bullet had rounded it
     up.
     ...
     **"a gate covers this" is itself a counted claim, and decays the
     ...
     Read what the gate says it read. Do not trust that it read
     ...
     read them: they are what it parsed on that run. The gate asserts no
     total. If the file states it six times and the receipt says five,
     the gate is green and the doc is wrong.

     Most gates in
     [`scripts/`](https://github.com/claymore666/docker-net-dhcp/tree/main/scripts)
     that discover what to check print the same kind of receipt: a
     count, or a line per item inspected. A few print a bare `PASS`, and
     for those the coverage is not visible at all without reading the
     script. Either way: when a gate's coverage is load-bearing for a
     claim in these docs, quote what it read. The fact that it exists is
     not the evidence.
   ...
   **The badge answers are documentation too.**
   [`.bestpractices.json`](https://github.com/claymore666/docker-net-dhcp/blob/main/.bestpractices.json)
...
4. **Add a `## vX.Y.Z` section** to
   [`RELEASE_NOTES.md`](https://github.com/claymore666/docker-net-dhcp/blob/main/RELEASE_NOTES.md),
   **above the previous version's section**. Summarise what's changing
   ...
   House style, applied by default on every release. A release note is
   reference material an operator scans. v1.8.0 shipped at 90KB of prose
   ...
   - Prefer a named list over a count. Write "six (#720, #721, ...)";
     "six of the ten" hides a wrong number.
   ...
   **Credit outside contributions by name**, the way the v1.0.0 notes
   do. Look them up; do not rely on recall. Almost every PR here is
...
6. **Open the release PR `dev` → `main`** with title
   ...
   **Milestone scope** workflow
   ([`.github/workflows/milestone-scope.yml`](https://github.com/claymore666/docker-net-dhcp/blob/main/.github/workflows/milestone-scope.yml),
   [`scripts/check-milestone-scope.sh`](https://github.com/claymore666/docker-net-dhcp/blob/main/scripts/check-milestone-scope.sh))
   ...
   **with** `in-dev` means the work shipped and the label is stale (drop
   ...
   ([`scripts/coverage-ratchet.sh`](https://github.com/claymore666/docker-net-dhcp/blob/main/scripts/coverage-ratchet.sh)
   vs
   ...
   **Release PR blocked on a check that has no run.** A required check
   ...
   presence** check
   ([`.github/workflows/coverage-presence.yml`](https://github.com/claymore666/docker-net-dhcp/blob/main/.github/workflows/coverage-presence.yml),
...
10. **Confirm the GitHub Release**. The `github-release` job now cuts it
    ...
    releases](verifying-releases.md). Step 4's notes must therefore
...
10b. **Nothing to refresh, and that is a change from 1.x.** Through
   ...
   **Where the digests are now.** In the signed checksums manifest,
   ...
   [`docs/verifying-releases.md`](verifying-releases.md) tells the
...
11. **Fast-forward `dev` to `main`** so the release commit (version
   ...
   **CI warns, but it cannot block.**
   [`.github/workflows/release-backmerge.yml`](https://github.com/claymore666/docker-net-dhcp/blob/main/.github/workflows/release-backmerge.yml)
   runs
   [`scripts/check-release-backmerge.sh`](https://github.com/claymore666/docker-net-dhcp/blob/main/scripts/check-release-backmerge.sh)
   in two modes:
...
12. **Prune merged branches.** The repo has *Automatically delete head
   ...
   the `upstream/*` refs (those are the original fork's remote).
