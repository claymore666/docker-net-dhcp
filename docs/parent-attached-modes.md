...
6. On `docker stop`, libnetwork calls `Leave` → the persistent client is
   ...
   a restart to do so (v2.1.1+, #962). A network created with
   `release_lease=on_remove` keeps both, and a sweep hands the addresses
   back about a minute later if no container has claimed them (v2.2.0+,
   #984).
