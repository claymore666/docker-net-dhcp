...
- **The container's kernel is put in charge of advertisements, and kept
  ...
  route disappeared, while on-link traffic kept working, which is what
  made it hard to spot. **What removed the route is not established, and
  this page deliberately does not name a cause.** Measured on a
  container before the fix: the route was installed from an
  advertisement carrying `expires 1799sec`, was still present 15 seconds
  after start, and was gone by 150 seconds — so its disappearance is not
  explained by the advertised router lifetime running out. What the fix
  rests on is narrower and is not in doubt: with advertisement
  processing off, nothing in the container could reinstall the route,
  whatever removed it. The DHCPv6 client now runs with `accept_ra=2`,
