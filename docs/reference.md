...
| `DOCKER_HOST` | *(empty)* | (2.0 beta+) Docker API endpoint. Empty means the socket `config.json` bind-mounts, which is what every installation before this setting used and what an operator who sets nothing keeps. Set it to a read-only socket proxy to reduce the one grant that makes a compromise of the plugin equivalent to root on the host — the plugin issues only `GET` and `HEAD`, refuses anything else before sending it, and counts the refusal as `docker_api_non_get_refusals`. The allowed paths and a worked example are in [SECURITY.md](https://github.com/claymore666/docker-net-dhcp/blob/main/SECURITY.md). A TLS endpoint is not supported: nothing here reads `DOCKER_CERT_PATH`, so the proxy must be a unix socket or a plain TCP endpoint. |
