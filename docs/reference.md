...
announcement 2 seconds after the first), and the plugin keeps 4 for its
answer, so the DHCP attempt gets 17. With the option on, `lease_timeout`
defaults to `17s` and a longer value is refused at `docker network
create`. In `conflict_check=wait` that funds one acquisition and not the
recovery from a conflict that the `34s` default funds. With no server on
the segment, the container starts about 22 to 26 seconds after its
...
[IPAM mode](#address-allocation); and a `lease_timeout` over `17s`.
