...
- **A network that offers no DHCPv6 address no longer refuses the
  ...
  created without a v6 lease and the container's own kernel forms a
  SLAAC address from the advertised prefix. The plugin does not manage
  that address and `docker inspect` does not report it.
