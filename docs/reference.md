...
| Advertised nothing at all | Starts the endpoint without a DHCPv6 address, and logs a warning. **In every `ipv6_mode`, including `slaac` and `auto`**, where a segment with no router leaves those modes no source of an address at all: they are tolerated here rather than failed because the address they would form is not installed yet (see the last paragraph of the `ipv6_mode` row), so failing the endpoint would refuse a container for the absence of something this release does not deliver. That changes with [#818](https://github.com/claymore666/docker-net-dhcp/issues/818) | `dhcpv6_no_router_advert` |
...
Since v2.2.0 the plugin reads the Router Advertisement itself and puts
its gateway, MTU, routes and resolvers into the Join answer on these
rows too, so an endpoint with no DHCPv6 lease still gets an IPv6 route
on a segment that advertises one. That is a Join-time answer only: with
no lease there is nothing for the plugin's live watch to follow, so a
later advertisement on such a segment is not applied until the container
is recreated.

**It does not get a global IPv6 address.** The guard writes
`autoconf=0`, so the container's kernel forms no address from the
advertised prefix. On a stateless or SLAAC segment that leaves a
link-local address and a route, and any global IPv6 address has to come
from somewhere else until the plugin forms one itself
([#818](https://github.com/claymore666/docker-net-dhcp/issues/818)).
Before v2.2.0 the container's kernel formed it.
