...
| `orphan_releases_suppressed` | no | (v1.9.0+) Orphaned-lease reclaims that stood down because a tombstone had already reserved that endpoint's address for the restart that was coming (#800). The two are contradictory instructions about one lease — keep it for the container, hand it back to the server — and before this the plugin issued both at once. Standing down is the outcome that keeps the address, so this is the mechanism working, not a fault. Expect it to track your container-restart rate; a host that never restarts containers reads zero. |
| `tombstones_suppressed` | no | (v1.9.0+) The other side of the same arbitration (#800): the reclaim got there first and handed the lease back, so no tombstone was written and that container restarts onto a **new MAC and address** instead of its own. Nothing is broken — the lease really was returned, and promising an address the server is free to reallocate is the defect this removes — but restart stability was lost for that container. Unlike its twin above this one costs something, so read a climbing rate as containers whose renewal client never bound before they went away, and look at `orphaned_leases_released` alongside it. |
