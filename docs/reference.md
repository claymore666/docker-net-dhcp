...
Ten counters carry a `family` label: `leases_obtained`,
`leases_renewed`, `lease_changed`, `renewals_unanswered`,
`dhcp_timeouts`, `naks_received`, `client_stop_failures`,
`address_conflicts`, `releases_sent` and `release_failures`:
...
Each of the ten has a `_v4` and a `_v6` field in
