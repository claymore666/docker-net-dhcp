...
| `conflict_probes_dispatched` | no | (v1.10.0+) Conflict probes launched, counted at the moment of launch rather than when they finish. This is the **population** `address_conflict_probes` should be read against. Do not read it against `leases_obtained`: that counter is incremented by the persistent DHCP client when it binds, which includes endpoints this plugin process never created — after a restart, recovery re-attaches an existing endpoint and its client binds, with no probe dispatched because there was no endpoint creation to dispatch one. Comparing the two then reports the detector as dead when nothing is wrong (#881). |
| `conflict_probes_settled` | no | (v1.10.0+) Conflict probes that reached a terminal outcome — a verdict, a recorded failure, or an exit with nothing to probe. Equal to `conflict_probes_dispatched` once the plugin is idle; **below it means probes are still running**, and a reading of `address_conflict_probes` taken at that moment is not yet a result. The probe is asynchronous and nothing in the request path waits for it, so this pair is the only way to tell "still working" from "never ran" (#881). |
