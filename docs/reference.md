...
| `ipam_reserve_joined` | no | warn | *(v2.1.0+)* Address requests that found a DHCP exchange already running for the same endpoint and waited for its result instead of starting a second one. Joining is the mechanism working and is not `healthy`-affecting: the alternative is two exchanges and two server leases for one container, of which the second would never be released. It is worth investigating because of what makes it happen: Docker re-sends a request body only when its own call to the plugin timed out, so a rise means the plugin call timeout (30s by default, set with `--timeout` when the plugin is enabled) is below what a DHCP acquisition on this segment costs. Raising it, or fixing the slow server, removes the duplicate requests. |
