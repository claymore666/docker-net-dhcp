...
| `acd_conflicts_detected` | no | — | (v2.0.0) Conflicts counted by the DHCP client's own state machine. It is the **same population** as `address_conflicts_v4`, counted inside the client rather than from the events it emitted, and the two are expected to be equal. It is **not** comparable to the unsuffixed `address_conflicts`, which carries the DHCPv6 share as well; DHCPv6 conflicts come from Duplicate Address Detection and this machine never sees them. A difference is a defect in the plugin's event handling, not a property of your segment — report it. Its `conflict_check=off` rule is its sibling's, not a different one: with no probe and no listener the only thing that can move either counter is a conflict reported to the client from outside it, and the plugin has no such caller today. |
...
| `address_conflicts_v4` | no | — | (v2.0.0) The RFC 5227 share of `address_conflicts`: an IPv4 address ARP found in use, §2.1 before it was used or §2.4 afterwards. **This is the half to compare against `acd_probes_sent` and `acd_conflicts_detected`** — both count ARP, and the unsuffixed total carries the v6 share too, so comparing against the total reports a plugin defect for every DHCPv6 conflict. `conflict_check` governs this half. |
| `address_conflicts_v6` | no | — | (v2.0.0) The DHCPv6 share: an address the container's kernel found on the link by Duplicate Address Detection (RFC 4862 §5.4), declined to the server under RFC 9915 §18.2.8. It is **not ARP**: no `acd_*` counter moves for it and `conflict_check` does not govern it — DAD is the kernel's, runs on every IPv6 address, and cannot be turned off from a network option. The client then acquires a replacement address and the plugin applies it to the container's interface; **Docker's record of the endpoint keeps the old address**, the same truthfulness gap `lease_changed` describes, so read `lease_changed_v6` beside this. |
