...
identity this binary was compiled with. The **check** column below says
which status each counter contributes: `fail` for the five that flip
`healthy`, `warn` for a counter whose row tells you to watch or alert
on it without calling it a fault, `—` for the informational majority,
which are not checks at all. `/metrics` carries `status` as
