...
starts the client first and hands the name over afterwards, which is
what [`hostnames_applied_late`](#pluginhealth) counts. On a busy daemon
...
| `hostname_apply_failures` | no | warn | *(v2.2.0+)* Container names the running DHCP client would not take, so the endpoint leases with no name in the DHCP server's table (#961). The daemon answered here; the client refused the handover. Three causes, all from the library: a name it will not put on the wire, a request queue that was full, and no running client left to give it to. Separate from `hostname_lookup_failures` because the two describe the same endpoint and their remedies are at opposite ends of the host. Read it beside `hostnames_applied_late`: a rise with no lookup failures next to it is this side and not the daemon's. |
