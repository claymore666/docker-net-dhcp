...
Request, Renew and Rebind, as RFC 4704 section 5 allows. The server
says in its Reply whether it took the update, and the plugin logs that
answer when the lease is bound: `The DHCPv6 server registers the AAAA
record for this name` when the S flag comes back set, a warning when it
comes back clear, and a note when the Reply carries no option 39 at
all. The plugin does no DNS update of its own, so a server that
