...
| `leases_obtained` | no | `dhcpcd` bind events (`BOUND`/`REBOOT`, and the v6 equivalents): initial bind or re-bind after NAK/lease loss. |
| `leases_renewed` | no | `dhcpcd` `RENEW`/`REBIND` events (v4 and v6). |
| `dhcp_timeouts` | no | DHCP-acquisition failures. `dhcpcd` only fires a hook when a *bound* lease lapses (`EXPIRE`) and is silent while it re-discovers, so this counter is bumped both by those `EXPIRE` events and by a periodic outage watchdog while the client is unbound and still trying — it keeps climbing for the duration of a server outage. |
...
| `naks_received` | no | (v1.0.0+) The server NAKed a renewal/rebind. `dhcpcd` recovers by re-acquiring, so each NAK is typically followed by `leases_obtained` — and, if the address moved, `lease_changed` — bumps. Climbing alongside `lease_changed` means containers are being re-addressed mid-life. |
