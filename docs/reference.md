...
3. **`—`** — everything else, which is the majority. A row that lands
   ...
   the clause that excluded it, and there are two such shapes: a
   counter whose imperative is to read it **against another counter**
   (its own value carries no verdict either way), and a counter whose
   normal reading is already non-zero (a `warn` check on it would fire
