...
| `ipam_reserve_joined` | no | warn | *(v2.1.0+)* Address requests that found a DHCP exchange already running for the same endpoint and waited for its result instead of starting a second one. Joining is the mechanism working and is not `healthy`-affecting: the alternative is two exchanges and two server leases for one container, of which the second would never be released. It moves when two address requests for the same pool and the same MAC are in flight at once, and the second waits for the first. It is **not** moved by Docker re-sending a request after its own plugin call timed out: that re-send carries no body (the daemon hands the same, already-drained reader to every attempt) and is refused before any handler runs, so raising `--timeout` does not change this counter. Worth investigating when it moves at all, as concurrency in the daemon's allocation path for one endpoint; the container start it belongs to still succeeds, so it is not something to alert on. |
