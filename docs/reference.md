...
| `ipv6` | all | `false` | upstream; **refused since v2.0.0** | **Setting this to `true` fails `docker network create`** with an error naming the 2.0 line and where DHCPv6 is tracked. The 2.0 DHCP client implements DHCPv4 only. Refused at network creation rather than at the first container, because an endpoint that quietly comes up without the address its network asked for is the failure this exists to prevent — see [DHCPv6](#dhcpv6-ipv6true). The 1.x line is where DHCPv6 works today. |
...
ones. v4 only — 2.0 leases no IPv6 addresses at all. Option 33, the
...
A static IPv6 request (`--ip6` / Interface.AddressIPv6) has no effect in
2.0, which runs no DHCPv6 exchange, so there is nothing to carry the
...
preferred address (v1.2.0+); DHCPv6 returns with #911, before `v2.0.0-rc2`.
...
equivalents (options 23 and 24) are not read, because 2.0 runs no DHCPv6
...
**2.0 leases no IPv6 addresses.** `-o ipv6=true` fails
`docker network create`; the client in this branch implements DHCPv4 only.
DHCPv6 returns with #911, before `v2.0.0-rc2`, and the 1.x line is where it works today.
...
- **`Join` returns success and the container starts — with an address
  ...
  **`ipv6_link_enable_failures`** and carries on — so that counter is *not*
  dead on 2.0, and the reason is the opposite of the obvious one: it is
...
| `leases_obtained` | no | — | Client bind events: an initial bind, or a re-bind after a NAK or a lease loss. v4+v6 aggregate — but 2.0 leases no IPv6, so this equals its `_v4` half. |
