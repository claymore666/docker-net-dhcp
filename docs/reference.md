...
| `stable_mac` | bridge, macvlan | `false` | v1.2.0 | Give every endpoint a deterministic MAC derived from the container's stable identity (Compose project + service + replica number, else container name) instead of a kernel-random one. A stable MAC ⇒ the upstream DHCP server hands back the same lease — and therefore the same IP — across a `compose up -d` recreate, with no static-IP pin and no server-side reservation. The matching DHCP option 61 (client-id) is derived from the same stable MAC so servers that key on client-id stay stable too. No-op for ipvlan (children share the parent's MAC; tracked as #219). Anonymous containers with no stable name fall back to a kernel-random MAC (a stable MAC from an unstable identity would drift anyway). Off by default; current behaviour unchanged unless opted in. |
| `mac_seed` | bridge, macvlan | identity-derived | v1.2.0 | Pin the identity that `stable_mac` hashes (`hash(networkID + seed)`) instead of inferring it from Compose/container-name. Only consulted when `stable_mac=true`. |
...
| `stable_mac_collisions` | no | (v1.2.0+) Deterministic-MAC perturbations on `stable_mac` networks: two endpoints' identity hashes collided in the ~46-bit MAC space (or a seed-input bug). Expected to stay `0`; the perturbed MAC is still valid and unique, so networking is unaffected — a non-zero value is a signal to investigate, not an outage. |
