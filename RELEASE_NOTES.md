...
| `--ipv6` on such a network is still refused, with a new message | The message says to drop `--ipv6` and set `-o ipv6=true` or `-o ipv6_mode=<mode>`, because the plugin allocates no IPv6 pool. A script that matches the old text no longer matches (#960). |
