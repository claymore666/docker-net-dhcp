...
|---|---|---|
| `network:host` | The plugin resolves and reads the parent link of every macvlan/ipvlan network, creates the host links it owns (a `vlan` sub-interface, and a bridge made from a spare NIC), and any `METRICS_ADDR` listener binds, on the host's own network namespace, not in a namespace of its own. | `pkg/plugin/parent_gate.go`, `pkg/plugin/vlan.go`, `pkg/plugin/bridge_own.go`, `cmd/net-dhcp/metrics.go` |
...
| `CAP_NET_ADMIN` | Every address, route, MTU and link change the plugin applies inside a container's network namespace, the parent/child link creation that attaches it, and on the host the `vlan` sub-interface and the bridge made from a spare NIC, with that NIC's move into it. | `pkg/plugin/dhcp_manager.go`, `pkg/plugin/netlink_seam.go`, `pkg/plugin/vlan.go`, `pkg/plugin/bridge_own.go` |
...
| `CAP_SYS_ADMIN` | `setns` into a container's network namespace on a locked OS thread, and into its mount namespace for a `resolv.conf` write; a private remount of `/proc/sys` on one thread, so the plugin can turn IPv6 on in a container link it prepares and off on a host link it creates. | `pkg/dhcp/chassis.go`, `pkg/plugin/resolvconf.go`, `pkg/plugin/v6_link.go`, `pkg/plugin/vlan.go` |
