...
func TestReportFQDN6_StaysQuietWhereNoNameWasSent(t *testing.T) {
	s := boundWith(&wire.ClientFQDN{Flags: wire.ClientFQDNFlagS, Name: "web1."})
	renewed := s
	renewed.Kind = lease.Renewed
	cases := []struct {
		name string
		opts *DHCPClientOptions
		ev   lease.Event
		...
		{"a renewal, not the bind", fqdnOpts6(), renewed},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			hook := captureLog(t)
			...
			if got := hook.AllEntries(); len(got) != 0 {
				t.Errorf("logged %q; a network that sent no option 39 has no answer to report, and a renewal "+
					"repeats the bind's", got[0].Message)
			}
		})
	}
}

// Both v6 loops report: the one-shot acquisition at CreateEndpoint and the persistent client after Join (#1029).
func TestReportFQDN6_BothLoopsReportTheBind(t *testing.T) {
	...
	t.Run("the persistent client", func(t *testing.T) {
		...
		src <- boundWith(answer)
		close(src)
		c.translate()
		if got := fqdnEntries(hook); len(got) != 1 || got[0].Data["fqdn_name"] != answer.Name {
			t.Errorf("the persistent client's bind logged %d option 39 entries, want one naming %q", len(got),
				answer.Name)
		}
	})
