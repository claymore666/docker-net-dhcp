...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "release_failures", counter: true, warn: true, unit: "releases", action: "A lease this network asked to hand back did not go on the wire, so that address stays leased until it expires on the server. Read it against releases_sent and check whether the client was still holding its binding at teardown.", help: "Attempts to hand a lease back that put no message on the wire: the client held no binding, the send failed, the request was dropped from a full queue, or a DHCPv6 address could not be taken off the link first (RFC 9915 section 18.2.7 requires that before the exchange). The address is left to expire on the server's clock, which is what a release_lease=never network does on every teardown.", field: "release_failures", v4field: "release_failures_v4", v6field: "release_failures_v6"},
