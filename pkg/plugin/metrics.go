...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "address_conflicts", counter: true, healthy: true, help: "Leased addresses RFC 5227 found already in use by another host, over the whole life of the lease: section 2.1's probes before the address is used and section 2.4's listener afterwards. Healthy-affecting. Moves in conflict_check=wait and =async. In =off the client neither probes nor listens, so this moves only for a conflict reported to the client from outside it, which no code path in this plugin does today.", field: "address_conflicts"},
		...
		{name: "sandbox_key_entry_failures", counter: true, help: "Attempts to enter a container network namespace through the sandbox key that were refused. EXPECTED, once per container attach, on a stock engine: the daemon bind-mounts each sandbox netns after the plugin's own /var/run/docker mount was taken, so the key resolves to the placeholder file underneath and the container PID route carries the attach. Nothing is degraded and no action is indicated. Read sandbox_key_not_permitted, sandbox_key_not_a_namespace, sandbox_key_wrong_ns_type and sandbox_key_unavailable to see which refusal this was; they sum to this counter.", field: "sandbox_key_entry_failures"},
		{name: "sandbox_key_not_permitted", counter: true, help: "Key-route refusals because the sandbox key did not name an entry of a directory this plugin accepts (/var/run/docker/netns, /run/docker/netns). This is the arm that is NOT expected: a daemon started with a non-default --exec-root publishes keys elsewhere, and the remedy is a change to this plugin rather than to the host. An arm of sandbox_key_entry_failures.", field: "sandbox_key_not_permitted"},
