...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "router_advert_guard_failures", counter: true, help: "Steps of the DHCPv6 Router Advertisement guard that did not take: a sysctl write that failed, or a read-back holding something other than what was written. Three knobs (accept_ra=2, autoconf=1, keep_addr_on_down=1), two steps each. DHCPv6 carries no route of its own, so a container whose kernel is not processing advertisements looks healthy and then loses its route when the advertisement it already has expires. It does not count a privileged process inside the container writing the settings back.", field: "router_advert_guard_failures"},
		...
		{name: "ipam_reserve_joined", counter: true, warn: true, unit: "requests", action: "Two address requests for the same pool and MAC were in flight at once and the second waited for the first. Worth investigating as concurrency in the allocation path; it is not the daemon's re-send after a plugin-call timeout, which carries no body and is refused before any handler runs, so raising --timeout does not affect it.", help: "Address requests that joined an exchange already running for the same endpoint instead of starting a second one.", field: "ipam_reserve_joined"},
