...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "address_conflicts", counter: true, healthy: true, help: "Leased addresses RFC 5227 found already in use by another host, over the whole life of the lease: section 2.1's probes before the address is used and section 2.4's listener afterwards. Healthy-affecting. Moves in conflict_check=wait and =async; in =off only for a conflict reported from outside the client.", field: "address_conflicts"},
		...
		{name: "acd_conflicts_detected", counter: true, help: "Conflicts the DHCP library itself counted. It is the same population as address_conflicts, counted inside the state machine rather than from the events it emitted, and the two are expected to be equal; a difference is a defect in the plugin's event handling, not a property of the segment. Moves in every mode, including conflict_check=off for a conflict reported from outside the client.", field: "acd_conflicts_detected"},
