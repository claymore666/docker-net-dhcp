...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "router_advert_guard_failures", counter: true, help: "Steps of a DHCPv6 client's Router-Advertisement guard that failed. The guard puts the container's kernel in charge of router discovery and prefix processing -- the only source of an IPv6 default route and of on-link determination, on a managed DHCPv6 segment as much as a stateless one -- and stops dhcpcd turning that off again. A failed step degrades rather than refusing the endpoint, and it is invisible in behaviour until the advertisement nothing refreshed expires and off-link IPv6 stops while on-link still works. Counts steps, not clients (#875).", field: "router_advert_guard_failures"},
