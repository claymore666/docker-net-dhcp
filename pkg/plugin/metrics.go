...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "hostnames_applied_late", counter: true, help: "Container names given to a DHCP client that was already leasing, which is how an endpoint is named without the attach waiting for the daemon (#961). The mechanism working, and the denominator for hostname_lookup_failures and hostname_apply_failures: a host that attached nothing has zeros everywhere. Counts non-empty names only, and v4 only, because this plugin sends no name option for DHCPv6.", field: "hostnames_applied_late"},
