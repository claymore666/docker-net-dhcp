...
func metricDefs() []metricDef {
	return []metricDef{
		...
		{name: "ipam_reserve_joined", counter: true, warn: true, unit: "requests", action: "An address request arrived for an endpoint whose DHCP exchange was already running, which the daemon does only when its own plugin call timed out. Worth investigating: the plugin-call timeout is set below what an acquisition on this segment costs.", help: "Address requests that joined an exchange already running for the same endpoint instead of starting a second one.", field: "ipam_reserve_joined"},
