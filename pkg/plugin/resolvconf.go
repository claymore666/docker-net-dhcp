...
//   - search-domain handling is deliberately minimal: we emit the
//     DHCP `domain` (option 15) as `search`. DHCP option 119 (Domain
//     Search List, multi-entry) is deferred to T2-2.
func writeContainerResolvConf(pid int, dns []string, searchDomain string) error {
	if len(dns) == 0 {
	...
	writeErr := os.WriteFile("/etc/resolv.conf", buildResolvConf(dns, searchDomain), 0644)
...
func buildResolvConf(dns []string, searchDomain string) []byte {
